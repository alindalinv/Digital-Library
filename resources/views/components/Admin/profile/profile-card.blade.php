...
<div class="mb-6 rounded-2xl border border-gray-200 p-5 lg:p-6">
    <div class="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div class="flex w-full flex-col items-center gap-6 xl:flex-row">
            ...
            <div class="h-20 w-20 shrink-0 overflow-hidden rounded-full border border-gray-200">
                <img src="{{ $avatar }}" alt="{{ $displayName }}" class="h-full w-full object-cover"
                    onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($displayName) }}&background=465fff&color=ffffff&bold=true';" />
            </div>
            ...
            <div class="order-3 w-full xl:order-2">
                ...
                {{-- User Meta (Role | Job | Organization) --}}
            ...
            @if ($user->hasSocialLinks())
                <div class="order-2 flex items-center gap-2 xl:order-3 xl:ml-auto">
                    ...
                    {{-- Facebook --}}
                    @if ($user->facebook)
                        <a href="{{ $user->facebook }}" target="_blank" rel="noopener noreferrer" aria-label="Facebook"
                    ...
                    {{-- X / Twitter --}}
                    @if ($user->twitter)
                        <a href="{{ $user->twitter }}" target="_blank" rel="noopener noreferrer" aria-label="X"
                            class="shadow-theme-xs flex h-11 w-11 items-center justify-center rounded-full border border-gray-300 bg-white text-gray-700 transition hover:bg-gray-50 hover:text-gray-800">
        ...
        {{-- ============ Edit Button ============ --}}
        <button type="button" @click="$dispatch('open-profile-header-modal')"
            class="shadow-theme-xs flex w-full items-center justify-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 lg:inline-flex lg:w-auto">
