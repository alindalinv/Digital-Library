        ...
        <div class="container">
            <div class="row g-3 mb-4">
                ...
                <div class="col-12 col-md-6">
                    <div class="card border-0 shadow-sm h-100 bg-primary text-white"><div class="card-body p-3 d-flex align-items-center justify-content-between"><div><p class="small text-white-50 mb-1">Keep reading</p><p class="fw-semibold mb-0">Find something new for your shelf.</p></div><span class="fs-2" aria-hidden="true">▣</span></div></div>
                </div>
            </div>
                ...
                @else
                    <div class="table-responsive">
                        <table class="table align-middle mb-0">
                            <thead class="table-light"><tr><th class="ps-4">Book</th><th>Status</th><th>Borrowed</th><th>Due</th></tr></thead>
                            <tbody>
                                @foreach($borrowings as $borrowing)
                                    ...
                                    <tr class="borrowing-row">
                                        <td class="ps-4"><div class="d-flex align-items-center gap-3"><img src="{{ $borrowing->book->cover_image ? asset('storage/' . $borrowing->book->cover_image) : asset('assets/frontend/images/default-book.png') }}" alt="{{ $borrowing->book->title }}" class="borrowing-cover rounded shadow-sm"><div class="min-w-0"><a href="{{ route('books.show', $borrowing->book->slug) }}" class="fw-semibold text-dark text-decoration-none">{{ $borrowing->book->title }}</a><div class="small text-secondary">{{ $borrowing->book->authors->pluck('name')->join(', ') }}</div></div></div></td>
