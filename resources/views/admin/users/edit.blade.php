...
@section('content')
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST"
        ...
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    @forelse ($adminRoles as $role)
                        ...
                        <label for="admin-role-{{ $role->id }}"
                            ...
                            <input id="admin-role-{{ $role->id }}" type="checkbox" name="admin_roles[]" value="{{ $role->id }}"
                                @checked(
                                    in_array(
                                        (string) $role->id,
                                        array_map(
            ...
            <div>
                <div class="mb-3">
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Member is a frontend role and always uses the
                        <code>web</code> guard.
                    </p>
                </div>
        ...
        <div class="flex justify-end gap-2">
            <a href="{{ route('admin.users.index') }}" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm
                            font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">
                Cancel
            </a>
            <button type="submit"
