...
@section('content')
    ...
    @php
        ...
        $selectedAuthors = old('authors', $book->authors->pluck('id')->map(fn ($id) => (string) $id)->all());
    @endphp
        {{-- Header --}}
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Edit Book</h3>
        ...
        <form method="POST"
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        ...
                        <select id="authors" name="authors[]" multiple size="6"
                            ...
                            @foreach($authors as $author)
                                <option value="{{ $author->id }}"
                                        @selected(in_array((string) $author->id, $selectedAuthors, true))
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="number" id="stock" name="stock"
                                   ...
                                   min="0" required aria-required="true" placeholder="10"
                                   class="{{ $baseInput }}">
                            @error('stock')
                                <p class="{{ $errorClass }}">{{ $message }}</p>
                            @enderror
                ...
                <div class="space-y-5">
                            ...
                            @else
                                <div id="coverPlaceholder"
                                    ...
                                    <div class="px-4">
                                        ...
                                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">No cover image</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                        ...
                        <select id="status" name="status" required aria-required="true"
                            ...
                            @foreach (['published' => 'Published', 'draft' => 'Draft', 'archived' => 'Archived'] as $value => $label)
                                <option value="{{ $value }}"
                                        @selected(old('status', $book->status) === $value)
                                        class="dark:bg-gray-900">
                                    {{ $label }}
                                </option>
                            @endforeach
                    ...
                    {{-- Book Information --}}
                    <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                        <h4 class="mb-4 text-sm font-semibold text-gray-800 dark:text-white/90">Book Information</h4>
                        <div class="space-y-3 text-sm">
                            <div class="flex items-center justify-between gap-3">
                                <span class="text-gray-500 dark:text-gray-400">Book ID</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">#{{ $book->id }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-3">
            ...
            <div class="mt-6 flex flex-col-reverse gap-3 border-t border-gray-200 pt-5 sm:flex-row sm:items-center sm:justify-end dark:border-gray-800">
                ...
                <button type="submit" id="updateBookButton"
                    ...
                    <svg id="updateBookSpinner" class="hidden h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                    </svg>
                    <svg id="updateBookIcon" class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                    <span id="updateBookText">Update Book</span>
                </button>
            </div>
        </form>
    {{-- =========================================================
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form?.addEventListener('submit', (e) => {
                ...
                // Allow navigation now that we've intentionally submitted.
                isDirty = false;
            });
        });
    </script>
