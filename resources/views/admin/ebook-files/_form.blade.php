<div>
    <label for="book_id" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Book</label>
    <select id="book_id" name="book_id" required class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm dark:border-gray-700 dark:text-white">
        <option value="">Select a book</option>
        @foreach ($books as $book)
            <option value="{{ $book->id }}" @selected(old('book_id', $ebookFile?->book_id ?? $selectedBookId) == $book->id)>{{ $book->title }}{{ $book->isbn ? " ({$book->isbn})" : '' }}</option>
        @endforeach
    </select>
